...
| S2 Cards/Surface Cache | 组件落盘+host 集成完成，**Gate 未正式关闭** | A2 集成：LumenGI.cpp 接线 CardScene+SurfaceCache+Scheduler+Capture 3D pass（FBO-less raster、identity 实例缓冲、page budget UI）；root 修复 CardScene includes/unique_ptr/Vao；运行时 smoke 过（reload/resize/toggle、零 validation error）；CPU 39/39。缺 S2 Gate：card placement 可视化、coverage 定量、30 分钟 churn、capture 图像测试。 |
